...
##############################################Takes Stanford tokenized files as input and discards the files which doesnot have th query term

for filename in $(find $TOKENIZED_FILES_PATH -type f -iname "*.tokenized")
	do
		echo 'Processing '$filename' ......' >> $LOGFILE1
		IFS='\/'	
		patharray=($filename)
		lenparray=${#patharray[@]}
		...
		IFS=''
		outputfilename=$FILTERED_TOKENIZED_FILES_PATH'/'$optpersonname'/'$optclustername'/'$optfilename'.filtered'
		python $SOURCE_PATH'filterDiscarded.py' $filename $outputfilename $optpersonname		
		echo 'End of Processing '$filename >> $LOGFILE1
		echo >> $LOGFILE1
	done
